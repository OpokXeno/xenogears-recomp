...
| [`movement`](#movement) | 70 | Position, rotation, walkmesh, movement, and collision. |
| [`world`](#world) | 20 | Maps, encounters, triggers, transitions, and scene state. |
...
| [`visual`](#visual) | 42 | Fades, lighting, models, particles, effects, and video. |
...
| [`flow`](#flow) | 39 | Jumps, calls, waits, yields, returns, and termination. |
| [`event`](#event) | 67 | Operations that do not belong exclusively to another subsystem. |
...
|---|---:|---|---|---|---|
| `0B` | 3 | `actor.initialize_npc_actor(field_graphic)` | `0x800A1624` | `InitializeNpcActor` | initializes the current NPC from a selected field graphic, synchronizes placement, enables updates and visibility, and advances three bytes. |
...
| `1F` | 2 | `actor.set_actor_low_flags_from_packed_byte(packed_actor_flags)` | `0x8009E1A0` | `SetActorLowFlagsFromPackedByte` | replaces low actor flags from a packed byte. |
| `20` | 3 | `actor.set_current_actor_flags(actor_flags)` | `0x8009E10C` | `SetCurrentActorFlags` | decodes a script mask into current-actor flags. |
...
| `27` | 2 | `actor.stop_and_disable_actor_vm(actor)` | `0x8009DC4C` | `StopAndDisableActorVM` | stops movement and disables actor VM state. |
| `28` | 2 | `actor.enable_actor_vm(actor)` | `0x8009DBC8` | `EnableActorVM` | enables an actor VM. |
| `29` | 2 | `actor.disable_and_hide_actor_vm(actor)` | `0x8009DAC4` | `DisableAndHideActorVM` | disables, hides, and closes actor dialogue. |
...
| `52` | 2 | `actor.follow_actor(actor)` | `0x800980FC` | `FollowActor` | continues planar walking toward a selected actor until contact range is reached. |
| `53` | 4 | `actor.follow_actor_with_step_limit(actor, step_limit)` | `0x80098038` | `FollowActorWithStepLimit` | continues planar walking toward a selected actor until contact range or the scripted step limit is reached. |
...
| `6B` | 3 | `actor.rotate_actor_clockwise(turn_sector)` | `0x8009AB5C` | `RotateActorClockwise` | rotates actor clockwise. |
| `6C` | 3 | `actor.rotate_actor_counter_clockwise(turn_sector)` | `0x8009ABAC` | `RotateActorCounterClockwise` | rotates actor counter-clockwise. |
| `6F` | 2 | `actor.face_actor(actor)` | `0x8009A2A8` | `FaceActor` | turns the current actor toward the selected actor when that actor is valid. |
| `70` | 2 | `actor.face_party_member(actor)` | `0x8009A1E4` | `FacePartyMember` | turns the current actor toward the mapped party member when that member is present. |
| `7B` | 4 | `actor.decrease_party_hp(party_hp_amount, party_member_mask_index)` | `0x800969FC` | `DecreasePartyHp` | VM party-HP decrease handler. |
| `7C` | 4 | `actor.increase_party_mp(party_mp_amount, party_member_mask_index)` | `0x80096F18` | `IncreasePartyMp` | increases party MP. |
| `7D` | 4 | `actor.decrease_party_mp(party_mp_amount, party_member_mask_index)` | `0x80097010` | `DecreasePartyMp` | decreases party MP. |
| `7E` | 4 | `actor.increase_party_hp(party_hp_amount, party_member_mask_index)` | `0x80097108` | `IncreasePartyHp` | increases HP for each valid party member selected by the script mask, caps each result at maximum HP, and advances four bytes. |
| `89` | 6 | `actor.check_actor_distance(actor, distance, label)` | `0x80095E48` | `CheckActorDistance` | tests actor distance. |
| `8A` | 4 | `actor.check_actor_on_screen(actor, label)` | `0x80095C00` | `CheckActorOnScreen` | tests actor screen visibility. |
| `91` | 4 | `actor.check_party_member(party_character_id, label)` | `0x800964B0` | `CheckPartyMember` | tests party membership. |
| `92` | 1 | `actor.initialize_actor_scripts(...)` | `0x800A19B0` | `InitializeActorScripts` | clears and disables all eight script slots of the current actor, resets scheduler state and yields without advancing the instruction pointer; there is no fallthrough to the next byte. |
| `93` | 3 | `actor.add_current_actor_to_mecha_list(mecha_graphic)` | `0x800A1364` | `AddCurrentActorToMechaList` | initializes the current actor's base graphic, synchronizes its transform, assigns a mecha-list slot and model identifier, and increments the mecha count. |
...
| `B9` | 4 | `actor.check_available_party_member(availability_bit_index, label)` | `0x80096534` | `CheckAvailablePartyMember` | advances four bytes when the requested character is available, otherwise branches to the encoded destination. |
| `BA` | 2 | `actor.add_available_party_member(character_id)` | `0x800965A8` | `AddAvailablePartyMember` | sets the requested character's availability bit and advances two bytes. |
| `BC` | 1 | `actor.initialize_actor_sprite(...)` | `0x800A0D3C` | `InitializeActorSprite` | creates and initializes actor sprite state. |
| `C4` | 2 | `actor.rotate_actor_and_set_state_flag(actor)` | `0x80093E30` | `RotateActorAndSetStateFlag` | rotates an actor over 30 steps and sets its state flag. |
| `C5` | 2 | `actor.rotate_actor_and_clear_state_flag(actor)` | `0x80093FC0` | `RotateActorAndClearStateFlag` | applies the inverse rotation and clears its state flag. |
| `DD` | 6 | `actor.set_current_actor_blend_parameters(blend_mode, blend_value)` | `0x80091E00` | `SetCurrentActorBlendParameters` | replaces rendering blend bits 5 through 6 from the low two operand bits, stores the accompanying rendering value, and advances six bytes. |
| `E0` | 7 | `actor.set_selected_actor_blend_parameters(actor, blend_mode, blend_value)` | `0x80091E98` | `SetSelectedActorBlendParameters` | resolves an actor and, when valid, replaces its rendering blend bits and accompanying rendering value before advancing seven bytes. |
| `F8` | 4 | `actor.update_current_actor_flag_quarter(flag_quarter_and_action, flag_mask)` | `0x8008E59C` | `UpdateCurrentActorFlagQuarter` | sets or clears the selected lower or upper 16-bit quarter of either current actor flag group and advances four bytes. |
| `F9` | 2 | `actor.set_parent_actor(parent_actor)` | `0x8008DE64` | `SetParentActor` | assigns an actor parent. |
...
| `FE 03` | 4 | `actor.set_current_actor_uniform_scale(scale)` | `0x8008D0F4` | `SetCurrentActorUniformScale` | sets uniform actor scale, applies a three-quarter sprite scale, refreshes rotation, and advances three bytes. |
| `FE 04` | 4 | `actor.set_current_actor_sprite_geometry_scale(scale)` | `0x8008D26C` | `SetCurrentActorSpriteGeometryScale` | doubles the supplied value into the current actor's sprite geometry scale and advances three bytes. |
...
| `FE 08` | 8 | `actor.set_current_actor_axis_scales(x, y, z)` | `0x8008D180` | `SetCurrentActorAxisScales` | sets independent actor X, Y, and Z scales, restores the sprite scale to 0xC00, refreshes rotation, and advances seven bytes. |
...
| `FE 18` | 3 | `actor.add_immediate_party_character(character) staged goto staged already_present goto already_present;` | `0x8008BDD8` | `AddImmediatePartyCharacter` | reserves a staging slot and begins loading an immediate party character, or marks an already staged character as present. The staged path resumes at FE-relative +3; the already-present path skips two subsequent code bytes and resumes at +5. Those bytes are not parameters. |
...
| `FE 5C` | 3, 4 or 5 | `actor.load_current_actor_mecha(1, resource); / actor.load_current_actor_mecha_reinterpret(party_selector);` | `0x800A0FD8` | `LoadCurrentActorMecha` | waits for I/O, hides or frees the indexed mecha, asynchronously loads replacement files, then constructs and binds the replacement with the actor's scale and position. Mode 1 reads its resource word at FE-relative +6. Unsupported modes return at the subopcode and execute primary 5C, giving the combined escape a four-byte footprint. |
...
| Opcode | Bytes | DSL form | Handler | Original function | Behavior |
|---|---:|---|---|---|---|
| `10` | 2 or 9 | `movement.begin_actor_move(x, z, y); / movement.continue_actor_move(x, z, y); / movement.begin_actor_move(x, z, y, reserved_flags=reserved_flags); / movement.continue_actor_move(x, z, y, phase=phase);` | `0x80098C00` | `MoveActorToPosition` | initializes unrestricted interpolated actor movement. |
| `11` | 4 or 9 | `movement.begin_actor_move_with_step_limit(x, z, y, step_limit); / movement.continue_actor_move_with_step_limit(x, z, y, limit_if_uninitialized); / movement.begin_actor_move_with_step_limit(x, z, y, step_limit, reserved_flags=reserved_flags); / movement.continue_actor_move_with_step_limit(x, z, y, limit_if_uninitialized, phase=phase, initial_limit=initial_limit);` | `0x80098C3C` | `MoveActorToPositionWithLimit` | phase zero initializes bounded interpolated movement and advances nine bytes; nonzero phases continue it, reread coordinates nine bytes earlier on completion, and resume four bytes after the continuation. The wrapper reads a limit at current IP+11 when its counter is uninitialized. |
| `17` | 18 | `movement.setup_movement_bounding_zone(vertex_1_x, vertex_1_z, vertex_2_x, vertex_2_z, vertex_3_x, vertex_3_z, vertex_4_x, vertex_4_z)` | `0x8009E91C` | `SetupMovementBoundingZone` | allocates and loads four movement-boundary vertices. |
| `18` | 5 | `movement.set_actor_collision_dimensions(collision_half_width_x, collision_half_width_z, collision_height, solid_contact_range)` | `0x8009E83C` | `SetActorCollisionDimensions` | sets actor collision bounds. |
| `19` | 6 | `movement.set_coordinates_and_clear_movement_state(x, z)` | `0x8009E4BC` | `SetCoordinatesAndClearMovementState` | applies coordinates and clears movement state. |
| `1A` | 2 | `movement.set_walkmesh_at_current_position(walkmesh)` | `0x8009E428` | `SetWalkmeshAtCurrentPosition` | changes walkmesh at the actor's current position. |
| `1B` | 7 | `movement.set_walkmesh_and_coordinates(x, z, walkmesh)` | `0x8009E35C` | `SetWalkmeshAndCoordinates` | assigns walkmesh and evaluated actor coordinates. |
| `1C` | 4 | `movement.apply_single_coordinate(actor_y)` | `0x8009E2C8` | `ApplySingleCoordinate` | applies one evaluated actor coordinate. |
| `1D` | 7 | `movement.apply_three_immediate_coordinates(x, z, y)` | `0x8009E248` | `ApplyThreeImmediateCoordinates` | applies three immediate actor coordinates. |
| `21` | 3 | `movement.set_actor_movement_speed(movement_speed)` | `0x8009E094` | `SetActorMovementSpeed` | writes the evaluated speed to SceneActorRecord+0x76, propagates it to the actor's bound field entity, and advances three bytecode bytes. |
...
| `44` | 5 | `movement.move_actor_along_angle_with_step_limit(angle, step_limit)` | `0x80098184` | `MoveActorAlongAngleWithStepLimit` | continues planar walking along a scripted angle until the step limit expires or the generated target is reached. |
| `45` | 8 | `movement.move_actor_along_angle3d_with_step_limit(height_offset, angle, step_limit)` | `0x80097864` | `MoveActorAlongAngle3DWithStepLimit` | continues three-axis walking toward a point 32 units along a scripted angle with a vertical offset until the destination or step limit is reached. |
...
| `4A` | 6 | `movement.move_actor_to_position_unlimited(x, z)` | `0x80099980` | `MoveActorToPositionUnlimited` | starts planar movement toward evaluated absolute coordinates with no step limit and waits for arrival. |
| `4B` | 8 | `movement.move_actor_to_position_with_step_limit(x, z, step_limit)` | `0x80098430` | `MoveActorToPositionWithStepLimit` | continues planar walking toward scripted absolute coordinates until the destination or step limit is reached. |
| `4C` | 8 | `movement.move_actor_to_position3d(x, z, y)` | `0x800979F0` | `MoveActorToPosition3D` | continues three-axis walking toward scripted absolute coordinates until the destination is reached. |
| `4D` | 10 | `movement.move_actor_to_position3d_with_step_limit(x, z, y, step_limit)` | `0x80097954` | `MoveActorToPosition3DWithStepLimit` | continues three-axis walking toward scripted absolute coordinates until the destination or step limit is reached. |
| `4E` | 6 | `movement.move_actor_by_offset(x, z)` | `0x80098370` | `MoveActorByOffset` | continues planar walking toward coordinates relative to the starting position until the destination is reached. |
| `4F` | 8 | `movement.move_actor_by_offset_with_step_limit(x, z, step_limit)` | `0x80098274` | `MoveActorByOffsetWithStepLimit` | continues planar walking toward coordinates relative to the starting position until the destination or step limit is reached. |
| `50` | 8 | `movement.move_actor_by_offset3d(x, z, y)` | `0x800977A4` | `MoveActorByOffset3D` | continues three-axis walking toward a position relative to the starting coordinates until the destination is reached. |
| `51` | 10 | `movement.move_actor_by_offset3d_with_step_limit(x, z, y, step_limit)` | `0x800976A8` | `MoveActorByOffset3DWithStepLimit` | continues three-axis walking toward a position relative to the starting coordinates until the destination or scripted step limit is reached. |
| `54` | 5 | `movement.move_toward_actor_unlimited(actor, height_offset)` | `0x800975C0` | `MoveTowardActorUnlimited` | copies a selected actor's position into the movement target, uses an unlimited step count, moves toward the target, and advances five bytes upon completion. |
| `55` | 7 | `movement.move_toward_actor_with_step_limit(actor, step_limit, height_offset)` | `0x8009749C` | `MoveTowardActorWithStepLimit` | copies a selected actor's position into the movement target, initializes the current slot's step limit, moves toward the target, and advances seven bytes upon completion. |
| `57` | 2 or 11 | `movement.finish_ballistic_actor_move(x, z, y, false); / movement.finish_ballistic_actor_move(x, z, y, true); / movement.finish_ballistic_actor_move(x, z, y, false, phase=phase); / movement.finish_ballistic_actor_move(x, z, y, true, phase=phase);` | `0x80099214` | `ContinueBallisticActorMove` | initializes or continues a timed parabolic move to scripted coordinates, optionally derives elevation from the walkmesh, updates facing and position each frame, and supports walkmesh-only refresh mode. |
| `58` | 4 | `movement.set_current_actor_axis_rotation(axis_rotation, axis)` | `0x80094918` | `SetCurrentActorAxisRotation` | assigns one current-actor axis. |
...
| `5F` | 2 | `movement.set_immediate_actor_cardinal_direction(cardinal_direction)` | `0x8009AD6C` | `SetImmediateActorCardinalDirection` | rotates the current actor to the immediate world-relative cardinal direction. |
| `67` | 4 | `movement.set_actor_direction(actor, direction)` | `0x8009ABFC` | `SetActorDirection` | sets actor direction. |
| `69` | 3 | `movement.set_cur_actor_rotation(rotation_sector)` | `0x8009AC7C` | `SetCurActorRotation` | sets current rotation. |
| `7F` | 3 | `movement.set_walkmesh_material_motion_angle_offset(motion_angle_offset)` | `0x80095300` | `SetWalkmeshMaterialMotionAngleOffset` | sets the global angular offset added to the current walkmesh material's motion direction before its XZ vector is accumulated. |
| `80` | 5 | `movement.set_walkmesh_material_byte(walkmesh_material, byte_lane, material_value)` | `0x80092664` | `SetWalkmeshMaterialByte` | writes a resolved value to the selected byte lane of a walkmesh material flag and advances five bytes. |
| `81` | 5 | `movement.or_walkmesh_material_byte(walkmesh_material, byte_lane, material_value)` | `0x800926C8` | `OrWalkmeshMaterialByte` | applies a bitwise OR with a resolved mask to the selected byte lane of a walkmesh material flag and advances five bytes. |
| `82` | 5 | `movement.write_walkmesh_normal_byte(walkmesh_material, byte_lane) -> (walkmesh_normal_byte)` | `0x80093664` | `WriteWalkmeshNormalByte` | reads a selected byte from a packed walkmesh-normal entry, writes it to the requested script variable, and advances five bytes. |
| `83` | 5 | `movement.and_walkmesh_material_byte(walkmesh_material, byte_lane, material_value)` | `0x80092768` | `AndWalkmeshMaterialByte` | applies a bitwise AND with a resolved mask to the selected byte lane of a walkmesh material flag and advances five bytes. |
| `BB` | 2 | `movement.remove_available_party_member(character_id)` | `0x800965F4` | `RemoveAvailablePartyMember` | clears the requested character's availability bit and advances two bytes. |
| `BD` | 3 | `movement.increase_current_actor_rotation_x(rotation_delta)` | `0x80094A5C` | `IncreaseCurrentActorRotationX` | increments actor rotation X. |
| `BE` | 3 | `movement.decrease_current_actor_rotation_x(rotation_delta)` | `0x80094ACC` | `DecreaseCurrentActorRotationX` | decrements actor rotation X. |
| `BF` | 3 | `movement.increase_current_actor_rotation_y(rotation_delta)` | `0x80094B3C` | `IncreaseCurrentActorRotationY` | increments actor rotation Y. |
| `C0` | 3 | `movement.decrease_current_actor_rotation_y(rotation_delta)` | `0x80094BAC` | `DecreaseCurrentActorRotationY` | decrements actor rotation Y. |
| `C1` | 3 | `movement.increase_current_actor_rotation_z(rotation_delta)` | `0x80094C1C` | `IncreaseCurrentActorRotationZ` | increments actor rotation Z. |
| `C2` | 3 | `movement.decrease_current_actor_rotation_z(rotation_delta)` | `0x80094C8C` | `DecreaseCurrentActorRotationZ` | decrements actor rotation Z. |
| `E1` | 14 | `movement.move_or_clear_vram_rectangle(source_x, source_y, width, height, destination_x, destination_y)` | `0x80091BBC` | `MoveOrClearVramRectangle` | clears the rectangle defined by the final four operands when the first coordinate pair is zero, otherwise moves the rectangle defined by the first four operands to the final coordinate pair, then advances fourteen bytes. |
| `E8` | 7 | `movement.move_actor_and_set_state_flag(movement_step, duration, turn_angle_or_axis_mode)` | `0x80094158` | `MoveActorAndSetStateFlag` | moves an actor for a scripted step count and sets its state flag. |
| `E9` | 7 | `movement.move_actor_and_clear_state_flag(movement_step, duration, turn_angle_or_axis_mode)` | `0x800943AC` | `MoveActorAndClearStateFlag` | applies complementary movement and clears its state flag. |
| `EB` | 20 | `movement.compute_orbit_point_from_coordinates(origin_x, origin_z, origin_y, yaw, pitch, radius) -> (orbit_position_x, orbit_position_z, orbit_position_y)` | `0x800910C0` | `ComputeOrbitPointFromCoordinates` | builds a camera-scale-adjusted orbit point around explicit coordinates from yaw, pitch, and magnitude inputs, writes its X, Z, and Y coordinates, and advances twenty bytes. |
| `F6` | 2 | `movement.configure_actor_rotation_lock_mode(mode)` | `0x8008E8C8` | `ConfigureActorRotationLockMode` | clears rotation locking and residual motion for mode zero, locks and snapshots rotation for mode one, enables deferred motion cleanup for mode two, and advances two bytes. |
| `FA` | 5 | `movement.adjust_actor_axis_rotation(actor, rotation_axis, rotation_delta)` | `0x800947B0` | `AdjustActorAxisRotation` | adjusts a selected actor rotation axis. |
...
| `FE 1C` | 9 | `movement.set_actor_position3d_immediate(x, z, y)` | `0x80098A7C` | `SetActorPosition3DImmediate` | places the current actor at scripted XYZ coordinates and synchronizes its rendered and physical positions. |
| `FE 23` | 21 | `movement.move_party_to_formation(x1, z1, x2, z2, x3, z3, facing1, facing2, facing3)` | `0x8009B398` | `MovePartyToFormation` | moves the three party slots toward separate scripted positions and facings until all arrive, while the sentinel mode immediately normalizes their rotations. |
...
| `FE 68` | 7 | `movement.walk_player_to_position_and_wait(x, z)` | `0x80092C20` | `WalkPlayerToPositionAndWait` | waits for transition systems to become ready, walks the player toward resolved X and Z coordinates, preserves the player's preexisting control flag, and completes after reaching or failing to reach the destination. |
...
| `FE 72` | 11 | `movement.write_interpolated_angle(from, to, step) -> state` | `0x800988B8` | `WriteInterpolatedAngle` | interpolates between two scripted angles by a scripted step and writes the resulting angle to script memory. |
| `FE 73` | 13 | `movement.write_distance_between_2d_points(x1, z1, x2, z2) -> state` | `0x8009861C` | `WriteDistanceBetween2DPoints` | computes the planar distance between two scripted points and writes the result to script memory. |
...
| `FE 76` | 17 | `movement.write_distance_between_3d_points(x1, z1, y1, x2, z2, y2) -> state` | `0x80098738` | `WriteDistanceBetween3DPoints` | computes the spatial distance between two scripted points and writes the result to script memory. |
...
| `FE D7` | 7 | `movement.set_world_map_marker_position_xz(x, z); / movement.set_world_map_marker_position_xz(x, z, reserved=reserved);` | `0x80087AB8` | `SetWorldMapMarkerPositionXZ` | writes X and Z coordinates into the saved world-map marker position, clears Y and padding, marks the position valid, and advances six bytes after FE. Its masked readers use a control byte at FE-relative +10, beyond the declared seven-byte encoding; preserve the neighboring bytes. |
...
| `12` | 9 | `world.start_custom_field_transition(destination_field, entry_point, transition_mode, transition_duration)` | `0x80093200` | `StartCustomFieldTransition` | waits for coordinator readiness, stages the destination field and entry parameter, records the transition mode and fade length, and yields for in-place map replacement. |
...
| `47` | 6 | `world.walk_player_and_change_field(destination_field, entry_point) -> (map_entry_point)` | `0x80092EA0` | `WalkPlayerAndChangeField` | waits for transition systems, enables scripted player control, and walks the player toward the current actor's offset exit point while committing the pending field transition. |
| `56` | 10 | `world.start_world_map_transition(return_field, start_position, camera_yaw, world_map_mode)` | `0x80093014` | `StartWorldMapTransition` | waits for loading and audio readiness, persists current Field state, disables encounters, stores the return field, initial position, camera yaw, and World Map mode, requests the World Map handoff, and advances ten bytes. |
...
| `98` | 5 | `world.change_field_when_ready(destination_field, entry_point) -> (map_entry_point)` | `0x800932D0` | `ChangeFieldWhenReady` | waits for coordinator readiness, optionally saves the current field and directions, stages the destination field and entry parameter, marks replacement pending, then yields and advances five bytes. |
| `9D` | 4 | `world.interpolate_scene_scale(scene_scale, duration)` | `0x8009A34C` | `InterpolateSceneScale` | starts interpolation from the current scene scale to the scripted scale over the encoded duration. |
...
| `F7` | 5 | `world.configure_random_encounter_timers(encounter_timer_range, active_timer_count)` | `0x8008E85C` | `ConfigureRandomEncounterTimers` | evaluates the encounter timer range and active-timer count, caps the count at 32, and regenerates that many unique randomized countdowns. |
...
| `61` | 8 | `camera.set_camera_target_movement_from(x, z, y)` | `0x8008FE2C` | `SetCameraTargetMovementFrom` | sets camera-target origin. |
| `62` | 2 | `camera.set_camera_target_movement_dest_to_actor(actor)` | `0x8008FF04` | `SetCameraTargetMovementDestToActor` | targets an actor. |
| `63` | 8 | `camera.set_camera_target_movement_dest(x, z, y)` | `0x8008FF90` | `SetCameraTargetMovementDest` | sets camera-target destination. |
| `64` | 1 | `camera.reset_camera_pos_movement(...)` | `0x80090068` | `ResetCameraPosMovement` | resets camera-position movement. |
| `65` | 8 | `camera.set_camera_pos_movement_from(x, z, y)` | `0x800900C4` | `SetCameraPosMovementFrom` | sets camera-position origin. |
| `66` | 2 | `camera.set_camera_pos_movement_dest_to_actor(actor)` | `0x8009019C` | `SetCameraPosMovementDestToActor` | moves camera to an actor. |
| `68` | 4 | `camera.set_target_actor_camera_relative_direction(actor, direction)` | `0x8009AC34` | `SetTargetActorCameraRelativeDirection` | rotates a selected actor to a scripted cardinal direction relative to the camera. |
| `6A` | 3 | `camera.set_camera_relative_actor_direction(camera_relative_direction)` | `0x8009ACB4` | `SetCameraRelativeActorDirection` | maps a direction to camera-relative actor rotation. |
| `97` | 3 | `camera.set_automatic_camera_rotation_sector_mask(camera_sector_mask)` | `0x8009A634` | `SetAutomaticCameraRotationSectorMask` | sets the eight-sector mask that can trigger automatic camera orbit correction. |
...
| `9A` | 3 | `camera.leave_or_reacquire_follow_camera(camera_reacquisition_duration)` | `0x8008FC4C` | `LeaveOrReacquireFollowCamera` | in script mode, zero duration returns immediately to normal follow and advances six bytes, while nonzero duration starts asynchronous mode-2 reacquisition and advances three; an invocation already in mode 2 leaves the PC unchanged. |
| `9B` | 5 | `camera.set_camera_smoothing_divisors(target_smoothing_divisor, eye_smoothing_divisor)` | `0x8008FD40` | `SetCameraSmoothingDivisors` | sets independent target and eye smoothing divisors, normalizing either zero operand to one. |
...
| `A1` | 3 | `camera.set_blocked_camera_sector_mask(blocked_camera_sector_mask)` | `0x8009A670` | `SetBlockedCameraSectorMask` | sets the eight-sector mask used to reject or redirect manual camera orbit. |
| `A3` | 8 | `camera.set_camera_pos_movement_dest(x, z, y)` | `0x80090228` | `SetCameraPosMovementDest` | sets camera-position destination. |
| `A4` | 4 | `camera.interpolate_camera_dip(camera_dip, duration)` | `0x8009A490` | `InterpolateCameraDip` | starts interpolation from the current camera dip to the scripted dip over the encoded duration. |
| `A5` | 3 | `camera.write_camera_direction(...) -> state` | `0x8009A534` | `WriteCameraDirection` | writes camera direction. |
| `AA` | 2 | `camera.set_immediate_camera_relative_actor_direction(cardinal_direction)` | `0x8009ACEC` | `SetImmediateCameraRelativeActorDirection` | rotates the current actor to an immediate cardinal direction relative to the camera. |
| `AB` | 1 | `camera.reset_camera_movements(...)` | `0x80090300` | `ResetCameraMovements` | resets Field camera movements. |
| `AC` | 4 | `camera.start_camera_movement(camera_movement_mode, duration)` | `0x800903BC` | `StartCameraMovement` | starts Field camera movement. |
...
| `B5` | 5 | `camera.set_camera_direction_timed(camera_direction, duration)` | `0x8009B8E4` | `SetCameraDirectionTimed` | snaps the camera direction when interpolation is disabled, otherwise waits for an active rotation or starts a timed direction change, then advances five bytes and yields. |
| `B6` | 5 | `camera.interpolate_projection_depth(projection_depth, duration)` | `0x8009B6AC` | `InterpolateProjectionDepth` | applies or interpolates projection depth to the scripted value over the scripted duration. |
...
| `C7` | 3 | `camera.start_camera_dolly_mode0(dolly_duration)` | `0x8009B824` | `StartCameraDollyMode0` | starts a positive one-eighth-turn camera dolly rotation over the requested duration when no rotation is active, then advances three bytes and yields. |
| `C8` | 3 | `camera.start_camera_dolly_mode0_alias(dolly_duration)` | `0x8009B884` | `StartCameraDollyMode0Alias` | starts a positive one-eighth-turn camera dolly rotation over the requested duration when no rotation is active, then advances three bytes and yields. |
| `E6` | 9 | `camera.set_camera_limits(limit_x, limit_z, extent_x, extent_z)` | `0x80091A08` | `SetCameraLimits` | stores the signed camera-limit origin and extents while negating the final extent and advances nine bytes. |
| `EC` | 15 | `camera.compute_camera_orbit_point(camera_orbit_selector, yaw, pitch, radius) -> (camera_orbit_position_x, camera_orbit_position_z, camera_orbit_position_y)` | `0x80091318` | `ComputeCameraOrbitPoint` | computes a rotated camera endpoint. |
| `ED` | 8 | `camera.write_camera_movement_parameter(camera_vector_selector) -> (camera_parameter_x, camera_parameter_z, camera_parameter_y)` | `0x800915C4` | `WriteCameraMovementParameter` | writes a camera parameter. |
| `EE` | 3 | `camera.set_camera_movement_parameter(source_camera_vector_selector, destination_camera_vector_selector)` | `0x80091720` | `SetCameraMovementParameter` | sets a camera parameter. |
...
| `F2` | 9 | `camera.setup_camera_shake(shake_x, shake_z, shake_y, duration)` | `0x8008F90C` | `SetupCameraShake` | configures target X, Z, and Y shake offsets and per-frame deltas for a nonzero duration, with an extended return-to-zero phase when every target is zero. |
...
| `FE 48` | 9 | `camera.set_projection_angles(yaw, dip, depth)` | `0x8008B518` | `SetCameraProjectionAngles` | sets the three signed camera projection angles. |
...
| Opcode | Bytes | DSL form | Handler | Original function | Behavior |
|---|---:|---|---|---|---|
| `03` | 4 | `dialogue.open_dialogue_mode2(dialogue_id, dialogue_control)` | `0x8009C104` | `OpenDialogueMode2` | opens the selected dialogue block through the common creation path in fixed-size mode 2 using the trailing control byte. |
| `CF` | 5 | `dialogue.set_dialogue_window_immediate(window_x_half_pixels, window_y, window_width_three_pixel_units, window_height)` | `0x8009CE48` | `SetDialogueWindowImmediate` | stores immediate forced X and Y positions, width in three-pixel character units, and height, then advances five bytes. |
| `D0` | 11 | `dialogue.set_dialogue_window_evaluated(window_x, window_y, window_width_three_pixel_units, window_height, window_flags)` | `0x8009CEE0` | `SetDialogueWindowEvaluated` | evaluates and stores forced X and Y positions, width in three-pixel character units, height, and window flags, then advances eleven bytes. |
| `D2` | 4 | `dialogue.open_actor_dialogue_mode0(dialogue_id, dialogue_control)` | `0x8009C0B4` | `OpenActorDialogueMode0` | opens the selected dialogue block in actor-anchored mode 0 using the trailing control byte. |
| `D3` | 4 | `dialogue.open_dialogue_mode1(dialogue_id, dialogue_control)` | `0x8009C0DC` | `OpenDialogueMode1` | opens the selected dialogue block in window mode 1 using the trailing control byte. |
| `D4` | 5 | `dialogue.open_dialogue_at_actor_mode0(actor, dialogue_id, dialogue_control)` | `0x8009C01C` | `OpenDialogueAtActorMode0` | resolves the selected actor as the placement anchor for the current actor's mode-zero dialogue, skips six bytes for an invalid actor, or rewinds one byte to retry deferred creation. |
| `D6` | 3 | `dialogue.set_dialog_animation_speed(dialogue_animation_mode)` | `0x800925A0` | `SetDialogAnimationSpeed` | stores the dialog animation mode and selects eight, six, or four opening frames for modes zero, one, or two before advancing three bytes. |
| `E7` | 7 | `dialogue.set_extended_background_clear_color(background_clear_red, background_clear_green, background_clear_blue)` | `0x80091A78` | `SetExtendedBackgroundClearColor` | stores the red, green, and blue background clear components and advances seven bytes. |
| `F4` | 2 | `dialogue.close_dialogue_or_reset_window_config(mode)` | `0x8009BE9C` | `CloseDialogueOrResetWindowConfig` | control value zero requests closure of the actor's owned dialogue, while a nonzero value clears configured width, height, forced position, and window flags. |
| `F5` | 4 | `dialogue.open_centered_dialogue_mode3(dialogue_id, dialogue_control)` | `0x8009C12C` | `OpenCenteredDialogueMode3` | opens the selected dialogue block in centered mode 3 using the trailing control byte. |
| `FC` | 5 | `dialogue.open_dialogue_at_actor_with_copied_portrait(actor, dialogue_id, dialogue_control)` | `0x8009BF8C` | `OpenDialogueAtActorWithCopiedPortrait` | resolves the selected actor, copies its portrait ID into the current actor, and opens the current actor's mode-zero dialogue anchored to the selected actor; an invalid selector skips six bytes and deferred creation retries. |
...
| Opcode | Bytes | DSL form | Handler | Original function | Behavior |
|---|---:|---|---|---|---|
| `72` | 3 | `audio.play_music_mode_zero(music_id)` | `0x8008F724` | `PlayMusicModeZero` | waits for music authorization then selects request mode zero and processes the requested music ID. |
| `74` | 3 | `audio.play_sound_effect(sound_effect_id)` | `0x8008F668` | `PlaySoundEffect` | starts or stops the requested sound effect on channel 3 and advances three bytes. |
| `75` | 3 | `audio.play_music_mode_minus_one(music_id)` | `0x8008F76C` | `PlayMusicModeMinusOne` | waits for music authorization then selects request mode minus one and processes the requested music ID. |
| `FE 0E` | 6 | `audio.fade_music_volume(target, duration)` | `0x8008C84C` | `FadeMusicVolume` | transitions music volume to the requested target over the requested duration and retries when music startup is incomplete. |
| `FE 0F` | 7 | `audio.fade_music_pitch(target, duration)` | `0x8008C938` | `FadeMusicPitch` | transitions music pitch to a signed target over the requested duration and retries when music startup is incomplete. |
| `FE 10` | 6 | `audio.fade_music_tempo(target, duration)` | `0x8008CA60` | `FadeMusicTempo` | transitions music tempo to the requested target over the requested duration and retries when music startup is incomplete. |
| `FE 11` | 7 | `audio.fade_music_pan(target, duration)` | `0x8008CB4C` | `FadeMusicPan` | transitions music pan to a signed target over the requested duration and retries when music startup is incomplete. |
| `FE 12` | 4 | `audio.set_music_channel_mask(mask)` | `0x8008CC74` | `SetMusicChannelMask` | applies a channel-enable mask to the active music instance and updates affected voices. |
| `FE 13` | 6 | `audio.configure_actor_positional_sound(sound_id, volume)` | `0x8008CD48` | `ConfigureActorPositionalSound` | assigns the current actor's positional sound identifier and volume in mode 0, stops its previous channel, and marks zero identifiers inactive. |
| `FE 14` | 6 | `audio.configure_actor_positional_sound_mode80(sound_id, volume)` | `0x8008CDD4` | `ConfigureActorPositionalSoundMode80` | assigns the current actor's positional sound identifier and volume with mode 0x80, stops its previous channel, and marks zero identifiers inactive. |
...
| `FE 3D` | 11 | `visual.set_primary_mecha_matrix_row(row, x, y, z)` | `0x8008AEC8` | `SetPrimaryMechaMatrixRow` | writes three signed values into a selected row of the primary mecha field matrix. |
| `FE 3E` | 11 | `visual.set_secondary_mecha_matrix_row(row, x, y, z)` | `0x8008AFD8` | `SetSecondaryMechaMatrixRow` | writes three signed values into a selected row of the secondary mecha field matrix. |
...
| `FE 6E` | 5 | `visual.set_scene_angle_y(angle)` | `0x8008FABC` | `SetSceneAngleY` | assigns both scene Y-angle fields. |
| `FE 6F` | 9 | `visual.set_global_model_rotation(x, z, y)` | `0x8008B45C` | `SetGlobalModelRotation` | sets the three signed global model-rotation angles used to rebuild the rendering matrix. |
...
| `FE A0` | 13 | `visual.start_signed_2d_presentation(resource, configuration_1, configuration_2, configuration_3, path)` | `0x8008EA58` | `StartSigned2DPresentation` | starts a signed-parameter 2D presentation with fixed viewport defaults, or rewinds and yields until presentation resources are available. |
...
| Opcode | Bytes | DSL form | Handler | Original function | Behavior |
|---|---:|---|---|---|---|
| `71` | 3 | `battle.start_battle(battle_configuration)` | `0x80093568` | `StartBattle` | waits for coordinator readiness, stores the requested battle configuration, marks the battle handoff pending, yields, and advances three bytes. |
...
| Opcode | Bytes | DSL form | Handler | Original function | Behavior |
|---|---:|---|---|---|---|
| `34` | 5 | `inventory.write_inventory_object_quantity(inventory_object) -> (inventory_object_quantity)` | `0x80096214` | `WriteInventoryObjectQuantity` | writes the encoded object's quantity to the requested script variable, using zero when the object is absent, and advances five bytes. |
| `8B` | 5 | `inventory.check_inventory_object(inventory_object, label)` | `0x800962C0` | `CheckInventoryObject` | advances five bytes when the encoded object is present, otherwise branches to the encoded destination. |
| `8C` | 3 | `inventory.add_inventory_object(inventory_object)` | `0x8009631C` | `AddInventoryObject` | adds one unit of the encoded object and advances three bytes. |
| `8D` | 3 | `inventory.remove_inventory_object(inventory_object)` | `0x8009640C` | `RemoveInventoryObject` | decrements an encoded object's quantity, clears its identifier when the quantity reaches zero, and advances three bytes. |
| `8E` | 7 | `inventory.check_gold_amount(gold_threshold, label)` | `0x80095F24` | `CheckGoldAmount` | tests the current gold amount. |
| `8F` | 3 | `inventory.increase_gold(gold_delta)` | `0x80095FB8` | `IncreaseGold` | increases party gold. |
| `90` | 3 | `inventory.decrease_gold(gold_delta)` | `0x8009601C` | `DecreaseGold` | decreases party gold. |
...
| `FE 56` | 4 | `inventory.open_menu_mode1_with_selection(...)` | `0x80093930` | `OpenMenuMode1WithSelection` | copies the evaluated selection into menu/game-state fields, queues menu mode 1, yields, and advances three bytes; it does not write an encoded or fixed VM output slot. |
...
| `D5` | 3 | `input.set_controller_btn_mask(controller_button_mask)` | `0x80092628` | `SetControllerBtnMask` | sets controller mask. |
| `E2` | 5 | `input.check_current_input_exact(input_mask, label)` | `0x80096150` | `CheckCurrentInputExact` | conditionally branches on exact current input. |
| `E3` | 5 | `input.check_accumulated_input_exact(input_mask, label)` | `0x80096178` | `CheckAccumulatedInputExact` | conditionally branches on exact accumulated input. |
| `FE 6C` | 2 | `event.clear_controller_enable_flag_and_stop(); / event.keep_controller_enable_flag();` | `0x8008A5A0` | `ClearControllerEnableFlag` | tests the byte immediately after this two-byte instruction and clears controller enable when it is zero; advances one byte after the FE prefix, leaving the tested byte for subsequent dispatch. |
...
| `84` | 5 | `state.check_scenario_flags_less_than(scenario_flags, label)` | `0x80096644` | `CheckScenarioFlagsLessThan` | compares scenario flags. |
| `85` | 5 | `state.check_scenario_flags_greater_than(scenario_flags, label)` | `0x800966B4` | `CheckScenarioFlagsGreaterThan` | compares scenario flags. |
| `86` | 5 | `state.check_scenario_flags_equal(scenario_flags, label)` | `0x80096724` | `CheckScenarioFlagsEqual` | compares scenario flags. |
| `87` | 3 | `state.set_scenario_flags(scenario_flags) -> (story_progress)` | `0x80096790` | `SetScenarioFlags` | writes scenario flags. |
...
| `78` | 4 | `flow.wait_for_archive_file(archive, file_id)` | `0x800973A4` | `WaitForArchiveFile` | requests or waits for the selected archive file. |
...
| `A2` | 2 | `flow.wait_for_camera_animation_mask(camera_animation_mask)` | `0x8009A58C` | `WaitForCameraAnimationMask` | advances only after every camera animation flag selected by the immediate mask has cleared. |
...
| `B2` | 2 | `flow.yield_until_camera_animation_mask_clears(camera_animation_mask)` | `0x8009A5E0` | `YieldUntilCameraAnimationMaskClears` | yields script execution while any camera animation flag selected by the immediate mask remains set. |
...
| `EF` | 3 | `flow.wait_for_camera_movement(camera_movement_mask)` | `0x8008FA38` | `WaitForCameraMovement` | waits for camera movement. |
| `FB` | 5 | `flow.jump_if_indexed_bit_clear(indexed_flags, bit_index, label)` | `0x8008D780` | `JumpIfIndexedBitClear` | conditionally branches on an indexed bit. |
...
| `2C` | 2 | `event.play_animation(animation_id)` | `0x8009A130` | `PlayAnimation` | starts an actor animation. |
| `5D` | 2 | `event.play_animation_and_clear_completion(animation_id)` | `0x8009A174` | `PlayAnimationAndClearCompletion` | starts the immediate actor animation and clears its completion latch. |
| `6D` | 8 | `event.cos(angle, magnitude) -> (cosine_result)` | `0x8009A6AC` | `Cos` | Field script cosine helper. |
| `6E` | 8 | `event.sin(angle, magnitude) -> (sine_result)` | `0x8009A768` | `Sin` | Field script sine helper. |
| `79` | 1 | `event.restore_hp(...)` | `0x80097264` | `RestoreHp` | restores HP. |
| `7A` | 1 | `event.restore_mp(...)` | `0x800972AC` | `RestoreMp` | restores MP. |
| `94` | 5 | `event.set_and_pause_event_timer(timer_high_byte, timer_low_byte) -> (event_timer)` | `0x800945D4` | `SetAndPauseEventTimer` | packs two evaluated bytes into VM variable 0x0A, pauses the event timer, and resets its update divider. |
| `95` | 2 | `event.configure_event_timer(timer_control)` | `0x80094650` | `ConfigureEventTimer` | configures event-timer pause and count direction from the supplied control byte. |
...
| `A0` | 7 | `event.set_screen_geometry(camera_sector, camera_dip, projection_screen_distance)` | `0x8009BA7C` | `SetScreenGeometry` | sets depth cue, camera direction, and projection distance from three script operands, updates the geometry screen distance, and advances seven bytes. |
| `A9` | 2 | `event.setup_multichoice(first_choice, last_choice)` | `0x8009BC98` | `SetupMultichoice` | waits for the current dialogue to accept choices, configures the packed first and last choice indices, initializes cursor state, and advances two bytes. |
| `CA` | 8 | `event.atan2(angle, magnitude) -> (atan2_result)` | `0x8009A824` | `Atan2` | Field script atan2 helper. |
...
| `D7` | 3 | `event.set_object_swivel_x_axis(rotation_angle)` | `0x800946BC` | `SetObjectSwivelXAxis` | selects object swivel around X and stores the evaluated angle. |
| `D8` | 3 | `event.set_object_swivel_y_axis(rotation_angle)` | `0x80094710` | `SetObjectSwivelYAxis` | selects object swivel around Y and stores the evaluated angle. |
| `D9` | 3 | `event.set_object_swivel_z_axis(rotation_angle)` | `0x80094764` | `SetObjectSwivelZAxis` | selects object swivel around Z and stores the evaluated angle. |
| `DB` | 5 | `event.set_deformation_strength(deformation_slot, strength)` | `0x80091F84` | `SetDeformationStrength` | caps a deformation strength at 0xFFF, stores it in the selected deformation slot when the current model supports deformation, and advances five bytes. |
| `E5` | 17 | `event.configure_fog(depth_cue_base_red, depth_cue_base_green, depth_cue_base_blue, fog_far_red, fog_far_green, fog_far_blue, fog_near_distance, fog_far_distance)` | `0x80091944` | `ConfigureFog` | stores the base depth-cue RGB color, far-fog RGB color, and near/far fog distances, enables fog, applies the new configuration, and advances seventeen bytes. |
| `EA` | 6 | `event.walk_player_to_aligned_exit(destination_field, entry_point) -> (map_entry_point)` | `0x80092DFC` | `WalkPlayerToAlignedExit` | waits for transition systems, enables scripted player control, and walks the player toward the current actor's aligned exit point while committing any pending field transition. |
| `F1` | 11 | `event.setup_rgb_calculation_mode1(semi_transparency_abr_mode, target_red, target_green, target_blue, duration)` | `0x8008B248` | `SetupRgbCalculationMode1` | configures RGB calculation mode 1 from five script parameters. |
...
| `FE 0C` | 14 | `event.set_shared_geometry_parameters(p1, p2, p3, p4, p5, p6)` | `0x8008CFEC` | `SetSharedGeometryParameters` | sets six signed shared geometry parameters from script operands. |
...
| `FE 77` | 3 or 12 | `event.manage_overlay_image_asset(0, resource);` | `0x8008A2E8` | `ManageOverlayImageAsset` | waits for archive I/O, then loads an indexed image into memory, uploads it with optional VRAM backup, or releases it according to the mode, and stops the current VM cycle. In short mode 0 it reads a control byte beyond the three-byte encoding; preserve the neighboring bytes. |
...
| `FE D2` | 4 | `event.skip_three_byte_instruction()` | `0x8008752C` | `SkipThreeByteInstruction` | advances past a three-byte no-op instruction. |

...
def emit_cold_cutover_table(
    ...
    lines.extend([
        ...
        "    const uint32_t normalized = pc & UINT32_C(0x1fffffff);",
        "    for (uint32_t index = 0u; index < (uint32_t)(sizeof(",
        "            xg_render_overlay_cutover_lookup) /",
        "            sizeof(xg_render_overlay_cutover_lookup[0])); ++index)",
        "        if (xg_render_overlay_cutover_lookup[index].pc == normalized)",
        "            return true;",
        "    return false;",
        "}",
        "",
        f"uint32_t xg_render_overlay_cutover_site_count(void) {{ return {len(sites)}u; }}",
        "",
        "bool xg_render_overlay_cutover_site_at(",
        "        uint32_t index, uint32_t *out_pc, uint32_t *out_instruction) {",
        "    for (uint32_t slot = 0u; slot < (uint32_t)(sizeof(",
        "            xg_render_overlay_cutover_lookup) /",
        "            sizeof(xg_render_overlay_cutover_lookup[0])); ++slot) {",
        "        const XgRenderOverlayCutoverSite *site =",
        "            &xg_render_overlay_cutover_lookup[slot];",
        "        if (site->pc == UINT32_C(0xffffffff)) continue;",
        "        if (index-- != 0u) continue;",
        "        if (out_pc != NULL) *out_pc = site->pc;",
        "        if (out_instruction != NULL) *out_instruction = site->instruction;",
        "        return out_pc != NULL && out_instruction != NULL;",
        "    }",
        "    return false;",
        "}",
        "",

...
This creates an important one-callback staging rule:
...
The offline renderer records starts and most automation at `callback + 1`, but
records key-off at the current callback.
...
WDS, SEDS, and SMDS do not contain that stream. The offline extractor currently
does not decode XA sectors or movie audio.

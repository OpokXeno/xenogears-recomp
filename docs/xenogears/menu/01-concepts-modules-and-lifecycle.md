...
Field, World, or resident state dispatcher
                  |
                  v
        Resident RunResidentMenu / DispatchMenuMode
...
| Component | Contract |
|---|---|
| `RunResidentMenu` | Creates `ResidentMenuState`, initializes display state, and enters `DispatchMenuMode`. |
| `DispatchMenuMode` | Dispatches mode `0..6` to the matching module entry. |
| `ResidentMenuState` | Shared root record containing graphics environments, command state, common selection state, and owner pointers. |
...
Resident `RunResidentMenu` at `0x8001C634` performs the following sequence:

1. Allocate and clear the `0x1E98`-byte `ResidentMenuState` root.
2. Publish its pointer at resident `0x800625A0`.
3. Store command `8`, the idle command, at `ResidentMenuState+0x325`.
4. Select heap user 2 for Menu allocations.
5. Select the second embedded graphics environment at `ResidentMenuState+0x120`.
6. Initialize frame counters, transition state, and draw control.
7. Build two 320 by 224 `GpuRasterEnvironment`/`GpuDisplayEnvironment` pairs and initialize the Menu view.
8. Install both graphics environment pairs and enable display output.
9. Call resident `DispatchMenuMode` at `0x8001C1A8`.
10. Complete resident post-return state handling.

`RunResidentMenu` creates the root, while the selected module releases it. This split
...
5. Resident `RunResidentMenu` creates `ResidentMenuState`, initializes graphics state, and
   calls `DispatchMenuMode`.
6. `DispatchMenuMode` calls the selected module entry.
...
11. The module releases `ResidentMenuState` and returns through `DispatchMenuMode`.
...
`ResidentMenuState` is the lifetime root, but every child has one explicit owner field.
...
field, so that field can remain stale until `ResidentMenuState` is released and must not
...
3. `ResidentMenuState` exists before a module entry runs.
...
7. `ResidentMenuState` is the final Menu allocation released.
...
| Resident `0x8001C634` | Create `ResidentMenuState` and run the resident Menu lifetime. |

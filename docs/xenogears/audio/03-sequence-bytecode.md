...
| Status | Meaning |
|---|---|
| Full | Behavior recovered from the retail driver and cross-checked against the current corpus |
| Bookkeeping | The driver changes musical counters that do not directly feed PCM |
| Partial | The command is understood, but some driver behavior remains unresolved |
| Context | Correct execution needs state outside the isolated resource |
| Unsupported | The command has a known size, but its behavior is not recovered |
| Driver no-op | The retail handler only consumes bytes and changes no playback state |
| Reserved | The command is reserved or rejected by the retail interpreter |
...
| `9C` | 4 | 0 | Partial | Calls a manager/global sound routine; exact side effects are not yet named |
| `9D` | 4 | 0 | Partial | Calls a manager/global sound routine; exact side effects are not yet named |
...
| `A1` | 2 | 0 | Partial | Relative tempo-state change; exact manager effect is not yet named |
...
| `A7` | 3 | 0 | Partial | Manager-volume interpolation; exact terminal behavior is not fully recovered |
| `A8` | 1 | 0 | Reserved | Reserved |
| `A9` | 2 | 244 | Full | Set gate mode |
| `AA` | 2 | 0 | Partial | Releases or reassigns a physical voice; exact selection behavior is not fully recovered |
...
The retail `0xB8` handler uses all three operands. Current analysis has recovered
the signed depth update. Every observed `0xB8` has zero delay and feedback, so the
effects of nonzero values remain unvalidated.

SEDS has no self-contained initial reverb state; interpreting `0xBA` or a later
`0xB8` therefore requires the live global reverb state established by the game.
...
the step and the terminal value. This differs from simply snapping to
`target << 8` and is retained as an observed driver quirk.

...
| `0x10` | 2 | SEDS-related ID | Stored by the manager; its exact game-level relation is not fully assigned |
...
The packed mapping value is interpreted as:

| Bits | Meaning |
|---|---|
| `0..7` | WDS preset index |
| `8..15` | Replacement note number |
| `16..23` | Unknown or reserved in the current analysis |
| `24..31` | Pan |
...
When `auxiliary_offset` is nonzero, the area beginning at that offset may contain
an auxiliary table or an internal score name. In observed files, a NUL-terminated
printable string can identify the score, but the broader purpose of the area is
not fully known.

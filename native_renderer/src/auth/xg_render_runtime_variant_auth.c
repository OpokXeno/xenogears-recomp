    ...
    for (uint32_t descriptor_index = 0u;
        ...
        if (descriptor->source_site_count != 0u &&
            normalized >= (descriptor->source_sites[0].pc &
                           UINT32_C(0x1fffffff)) &&
            normalized <= (descriptor->source_sites[
...
void xg_render_runtime_variant_classify_code_write(
    ...
    const uint32_t mask =
        xg_render_runtime_variant_descriptor_code_write_overlap_mask(
            address, size);
    ...
    *out_classification = (XgRenderMutationClassification){
        .properties = {
            ...
            .descriptor_mutation = mask != 0u,
            .semantic_authority_loss = mask != 0u,

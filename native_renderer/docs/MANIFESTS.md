...
| `xg_render_ui_owners.toml` | Declares the resource/UI owner catalog generated by CMake. |
...
5. Review the companion declarations against the canonical metadata. When the
   authenticated artifact is available, use the `validate` subcommand of
   `tools/native_render_runtime_variants.py` with the companion,
   `--canonical-manifest`, and `--artifact`.
   Declaration-only table generation is not artifact-byte validation.

## Overlay range manifest

File: `native_renderer/xg_render_overlay_ranges.toml`

`base_address` identifies the page-aligned runtime artifact authority.
`load_address` may be supplied when the raw image begins later inside that
artifact; code-range hashes and emitted source hooks are indexed from this
actual load address. When omitted, `load_address` equals `base_address`.
...
The static AOT build materializes the same authenticated range plan before
invoking `psxrecomp-game`; overlays with no matching ranges compile without a
plan, while matched overlays receive the nonempty plan explicitly.
...
| `<build-dir>/native_renderer/generated/xg_render_ui_owner_catalog.c` | UI owner catalog, canonical manifest, and overlay index. |
...
9. Review the generated metadata contract and source-observation bindings.

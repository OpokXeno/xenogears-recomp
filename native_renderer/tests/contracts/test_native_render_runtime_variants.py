...
class NativeRenderRuntimeVariantsTests(unittest.TestCase):
    ...
    def test_declared_descriptor_binds_all_canonical_overlays(self) -> None:
        canonical = tomllib.loads(CANONICAL_MANIFEST.read_text(encoding="utf-8"))
        self.assertGreater(len(canonical["overlay"]), 1)
        companion = tomllib.loads(COMPANION.read_text(encoding="utf-8"))
        self.assertEqual(
            companion["canonical"]["manifest_sha256"],
            hashlib.sha256(CANONICAL_MANIFEST.read_bytes()).hexdigest())
        with tempfile.TemporaryDirectory() as directory:
            output = Path(directory) / "runtime_variants.c"

            self.run_tool(
                "emit-declared", str(COMPANION), "--canonical-manifest",
                str(CANONICAL_MANIFEST), "--out", str(output))

            identity = hashlib.sha256(CANONICAL_MANIFEST.read_bytes()).digest()
            initializer = ",".join(f"0x{byte:02x}" for byte in identity)
            self.assertIn(
                f"{{{initializer}}}", output.read_text(encoding="ascii"))

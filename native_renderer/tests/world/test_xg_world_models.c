...
static int test_depth_cued_quad_right_cull_writes_first_xy(void) {
    ...
    CHECK(output.counter_incremented);
    CHECK(output.guest_ordering_table_written);
    CHECK(output.primitive.triangle_count == 2u);
    CHECK(output.guest_packet_word_write_mask ==
          output.packet_word_write_mask);
    return 1;
}
...
static int test_native_finalize_tracks_guest_count_separately(void) {
    ...
    for (index = 0u; index < 2u; ++index) {
        ...
        outputs[index].primitive.triangle_count = 1u;
    }
    ...
    outputs[1].ordering_table_written = false;
    outputs[1].guest_packet_word_write_mask = 1u;
    outputs[1].counter_incremented = true;
    outputs[1].guest_ordering_table_written = true;

    dispatch_output.source_index = dispatch.source_index;
    dispatch_output.processed_primitive_count = 2u;
    dispatch_output.accepted_primitive_count = 1u;
    dispatch_output.packet_side_effect_primitive_count = 2u;
    dispatch_output.emitted_count_delta = 2u;
    ...
    CHECK(commit.resident_emitted_count == 2u);
    CHECK(commit.entry_side_effects.resident_emitted_count == 2u);
    CHECK(commit.accepted_primitive_count == 1u);
    CHECK(probe.begin_count == 1u);
    CHECK(probe.stage_count == 2u);
    CHECK(outputs[0].primitive_staged);
    CHECK(outputs[1].primitive_staged);
    return 1;
}
